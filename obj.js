...
class ObjMesh
{
		...
		for ( var i=0; i<lines.length; ++i ) {
			...
			switch ( elem[0][0] ) {
				...
				case 'f':
					...
					this.edge_list[f[0]].add(f[1]).add(f[2]);
					this.edge_list[f[1]].add(f[2]).add(f[0]);
					this.edge_list[f[2]].add(f[0]).add(f[1]);
					for ( var j=3; j<f.length; ++j ) {
						this.edge_list[f[0]].add(f[j-1]).add(f[j]);
						this.edge_list[f[j-1]].add(f[j]).add(f[0]);
						this.edge_list[f[j]].add(f[0]).add(f[j-1]);
					}
					break;
			}
		}
	...
	/**
     * Return the neighbor vertices of this vertex in the mesh
     * @param {int} vert 
     * @returns {Set} of vertices
     */
	neighbors(vert){
		return this.edge_list[vert];
	}
